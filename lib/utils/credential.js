...
class CredentialManager {
  ...
  get() {
    return !this.credential ? this.create() : Promise.resolve(this.credential);
  }

  delete() {
    return !this.credential ? null : CredentialService.forceDelete(this.credential);
  }
...
class CredentialService {
  ...
  static detachAll(credential) {
    return Promise.resolve(credential)
      ...
      .then(associates =>
        this.detachAssociates(credential, associates.deviceIds, associates.policyIds)
      );
  }
  ...
  static getAssociateIds(credential) {
    return Promise.all([this.getDeviceIds(credential), this.getPolicyIds(credential)]).then(
      data => {
        return {
          deviceIds: data[0],
          policyIds: data[1]
  ...
  static detachPolicy(params) {
    return this._detachPolicy(params).catch(ex => {
  ...
  static detachDevice(params) {
    return this._detachDevice(params).catch(ex => {
      if (ex.code === 'ResourceNotFoundException') return params;
    });
  }
