    ...
    return super.init(opts).then(
      data => {
        ...
        this.eventSource.on('command', cmd => {
          ...
          if (cmd.method === 'control') {
            if (cmd.params[0].data) {
              console.log(cmd.params[0].data);
              let action = cmd.params[0].data;
              gpio.setup(action.pin, action[action.pin].direction)
                .then((response) => {
                  return gpio.write(action.pin, action[action.pin].value);
                })
                .then(data => {
                  cmd.success({data});
                }).catch(ex => {
          ...
          if (cmd.method === '_update') {
            if (this.relatedDevicesMap[cmd.deviceId]) {
              Promise.all([
                cmd.success(this.state),
                this.relatedDevicesMap[cmd.deviceId].update(cmd.params[0].data.custom)
              ]);
            }
